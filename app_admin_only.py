...
@app.route('/api/admin/generate-film-info', methods=['POST'])
def generate_film_info():
    """Генерирует описание фильма на основе его названия"""
    try:
        data = request.get_json()
        if not data:
            return jsonify({'success': False, 'error': 'No data provided'}), 400
        
        film_title = data.get('film_title', '').strip()
        if not film_title:
            return jsonify({'success': False, 'error': 'Film title is required'}), 400
        
        logger.info(f"🎬 Генерация описания для фильма: {film_title}")
        
        # Создаем промпт для генерации описания фильма
        film_info_prompt = f"""
Создай короткое описание фильма "{film_title}" для концертного анонса.

Требования:
- Расскажи, о чем фильм, какие идеи и вопросы он поднимает
- Какие эмоции и атмосферу создает
- СТРОГО НЕ БОЛЕЕ 200 символов
- ДВА завершенных предложения с точкой в конце каждого
- Яркий, эмоциональный язык
- На русском языке
- Без технических терминов
- Каждое предложение должно быть полным и завершенным

ВАЖНО: Убедись, что оба предложения полностью завершены и общая длина НЕ превышает 200 символов!

Формат: Два коротких предложения. Каждое заканчивается точкой.
"""
        ...
        try:
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            film_description = loop.run_until_complete(get_openai_response(film_info_prompt))
            loop.close()
            
            # Обрезаем до 250 символов
            if len(film_description) > 250:
                film_description = film_description[:247] + '...'
        ...
        logger.error(f"Ошибка в generate_film_info: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        })
...
def generate_film_description():
    """Генерировать красивое описание фильма на основе технического промта или миксированного текста"""
    try:
        data = request.get_json()
        if not data:
            return jsonify({'success': False, 'error': 'No data provided', 'message': 'Не переданы данные для генерации'}), 400
        
        technical_prompt = data.get('technical_prompt', '').strip()
        
        # Если промпт пустой, возвращаем информативную ошибку
        if not technical_prompt:
            logger.warning("⚠️ Пустой technical_prompt - возможно, нет миксированного текста")
            return jsonify({
                'success': False, 
                'error': 'Нет данных для генерации описания',
                'message': 'Сначала нужно обработать сообщения пользователей для создания миксированного текста'
            }), 400
        
        logger.info(f"🎬 Генерация описания фильма на основе текста: {technical_prompt[:100]}...")
        
        # Создаем промпт для генерации описания фильма
        film_prompt = f"""
На основе этого описания создай красивое короткое описание для концертного анонса:
...
- Это описание для концерта, где играют музыку к фильмам
        """
...
if __name__ == '__main__':
    ...
    # Запуск Flask приложения на порту из переменной окружения или 8000 по умолчанию
    port = int(os.getenv('PORT', '8000'))
    print(f"📊 Админ-панель запускается на порту: {port}")
