...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if context_message:
        ...
        try:
            ...
            evaluation_prompt = f"""
Ты - доброжелательный концертный анонсер на мероприятии. Пользователь ответил на твой вопрос о фильме.

Вопрос администратора: {context_message[:300]}

Ответ пользователя: {user_message}

Твоя задача:
1. Внимательно проанализируй ответ пользователя
2. Найди в нем что-то интересное, образное или эмоциональное
3. Поблагодари пользователя за его ответ искренне и тепло
4. Отметь конкретно, что тебе понравилось в его ответе (образ, ассоциация, эмоция)
5. Будь позитивным, воодушевляющим и поддерживающим

Важно:
- НЕ задавай дополнительных вопросов
- НЕ проси уточнений
- Сосредоточься на благодарности и позитивной оценке
- Даже если ответ краткий (одно слово), найди в нем ценность и интерес

Формат ответа: 2-3 предложения с искренней благодарностью и позитивной оценкой.

Примеры хороших ответов:
- "Вокзал - какой интересный образ! Действительно, атмосфера фильма напоминает путешествие и ожидание чего-то важного. Спасибо за ваш отклик! ✨"
- "Кирпичные стены - точно! Этот образ отлично передает суровую, но честную атмосферу фильма. Благодарю вас за такую яркую ассоциацию! 🎬"
"""
