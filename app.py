...
def admin_clear_all_chats():
    ...
    try:
        ...
        try:
            clear_chat_message = "🔄 История чата была очищена администратором. Пожалуйста, обновите страницу для корректного отображения."
...
Требования:
- Напиши ОДНО предложение длиной НЕ БОЛЕЕ 300 символов
...
- Сделай описание более подробным и атмосферным
- ОБЯЗАТЕЛЬНО соблюдай лимит в 300 символов

Пример стиля:
"Погружаясь в атмосферу грязных улиц Филадельфии, зритель оказывается в потных спортзалах, где каждое дыхание наполнено решимостью и стойкостью. Сырые кирпичи и металлические тренажеры становятся свидетелями беспощадных тренировок, окрашенных в цвета уличной холодной суеты и уютного теплого света спортивных залов."
"""
