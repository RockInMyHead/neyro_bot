...
Пример: "Море, шторм, корабль, приключения" """
    ...
    async def _generate_single_image(self, prompt: str, batch_id: str) -> str:
        ...
        try:
            # Создаем полный промпт для генерации изображения
            # Добавляем стилевые элементы к краткому промпту пользователей
            style_addition = " Мрачный кинематографичный реализм во вселенной Пиратов карибского моря; морская дымка, контраст, рим-свет; палитра: сталь/свинец воды, изумруд/бирюза, мох, мокрое дерево, патина бронзы, янтарные блики; фактуры: соль на канатах, камень, рваная парусина, брызги; широкий план, масштаб, без крупных лиц."
